...
CREATE VIEW [dbo].[WorkerDuplicates]
AS
...
LEFT JOIN (
			SELECT 
				...
				,CASE WHEN nsp.TEAM_CUSTOM IS NULL THEN p.TEAM_CUSTOM 
					ELSE nsp.TEAM_CUSTOM END AS TEAM_CUSTOM
				,CASE WHEN nsp.TEAMV2_CUSTOM IS NULL THEN CASE WHEN p.POSITIONID IS NULL THEN mdp.TEAMV2_CUSTOM 
					ELSE p.TEAMV2_CUSTOM END ELSE nsp.TEAMV2_CUSTOM END AS TEAMV2_CUSTOM
				...
				,CASE WHEN CASE WHEN nsp.JOBID IS NULL THEN pj.DESCRIPTION 
								ELSE nspj.DESCRIPTION END IS NULL THEN 
							CASE WHEN nsp.JOBID IS NULL THEN p.JOBID 
								ELSE nsp.JOBID END 
	...
	LEFT JOIN (
				...
				INNER JOIN ( 
							SELECT POSITIONID, MAX(VALIDTO) AS LR 
	...
	LEFT JOIN (
			SELECT CONVERT(decimal,e.PAYRATE) AS PAYRATE, e.POSITIONID
	...
	LEFT JOIN (
			...
			INNER JOIN ( 
						...
						) ed ON ed.PERSONNELNUMBER = lr.PERSONNELNUMBER 
								AND ed.EMPLOYMENTENDDATE = lr.LR
				) le ON le.PERSONNELNUMBER = w.PERSONNELNUMBER 
			LEFT JOIN HcmPersonCertificateStaging c ON c.certificatetypeid = 'MDBC' AND c.PARTYNUMBER = w.PARTYNUMBER
...
INNER JOIN DUPES ND ON ND.PERSONNELNUMBER = W.PERSONNELNUMBER
